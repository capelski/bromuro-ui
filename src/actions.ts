...
const addDisplayedJokeId = (displayedJokesId: number[], nextJokeId: number) => {
  if (displayedJokesId.indexOf(nextJokeId) === -1) {
    ...
    AsyncStorage.setItem('displayedJokesId', JSON.stringify(displayedJokesId)).catch(console.warn);
  }
};
...
export const loadMatchingJoke = (state: State, stateSetters: StateSetters) => {
  let offset = isNaN(state.searcherOffsets[state.filter]) ? 0 : state.searcherOffsets[state.filter];
